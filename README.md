...
- **项目级记忆**：工作区根目录的 `AGENTS.md` / `agents.md` 自动注入为项目指令；升级为可检索知识库（应用数据目录对应工作区的 `memory/` 多条目 + `memory_search` 关键词检索，`shared=True` 可写入全局 `~/.config/lumina/memory/` 跨项目复用），启动时注入条目索引
...
- **命令行入口自管理**：仅安装 exe 的用户首次启动时会提示添加命令行入口（`lumina chat` / `lumina run` 等），
  之后若命令行不可用会在设置按钮上显示红点引导，可在桌面设置中开关或移除
- **版本自检**：启动时检测 GitHub 最新发布版，发现新版本可一键跳转下载
- **多语言界面**：简体中文 / English，设置 → 通用 → 语言 即时切换（重启终端 / 新会话生效）
- **终端 shell 选择**：自动 / PowerShell / CMD / Bash，兼容的 shell 也会用于智能体工具调用
- **自定义智能体选择器**：设置 → 智能体 开启后，输入框可直接切换智能体，并按会话记忆上次选择
- **服务器状态指示灯与面板**：顶栏状态指示灯（本地可用时点亮）+ 状态面板，一键查看本地与所有已配置服务器的可用性
- **更新后发行说明**：升级到新版本后自动弹出版本说明，可在设置中关闭
...
| `LUMINA_VISION` | `false` | 多模态图片输入开关；需配合支持视觉的模型（DeepSeek 不支持） |
...
- 首次启动若检测不到命令行 `lumina` 会询问是否添加：确认后在
  `%LOCALAPPDATA%\LuminaCode\bin` 生成 `lumina.cmd` 并写入用户 PATH，之后即可在任意终端使用
  `lumina chat` / `lumina run "…"` 等指令（该 exe 同时内置了 CLI 入口）；可在「设置 → 桌面 → 命令行入口」关闭提醒或移除。
...
- 多模态输入：点击输入框左侧图片按钮粘贴 / 选择图片（最多 4 张、单张 ≤4MB），随消息发送给视觉模型；需在设置中开启 `LUMINA_VISION` 并配置支持视觉的模型（DeepSeek 不支持图片）
...
- 设置面板支持多语言切换（简体中文 / English）、终端 shell 选择、命令行入口 / 命令面板 / 智能体选择器 / 服务器状态指示灯 / 发行说明等开关
- 顶栏服务器状态按钮：点按弹出状态面板，实时显示本地服务与各配置服务器可用性
- 输入框智能体选择器（设置 → 智能体 开启）：快速切换智能体，选择按会话记忆
- 命令面板（`Ctrl+K`，设置 → 通用 可关闭）：快速新建会话、切换工作区、打开设置、切换终端、折叠侧边栏、刷新文件树、检查更新
...
├── logging_setup.py   # 日志持久化（应用数据目录对应工作区的 lumina.log）
├── mcp/               # MCP 客户端
└── skills/            # 技能加载器
```
